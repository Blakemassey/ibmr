...
###             current_agent = numeric, the id (a numeric value) of the current agent
###             add_agents = logical, whether or not new agents should be added to 'agents'
###             new_intput = not implemented yet, currently set to NULL
###  Returns: an 'agents' list
###  Notes: New columns can be via the "add_columns" vector w. The arguments 'current_agent' and
###         'add_agents' are currently designed for use in the indigo snake IBM where new agents
###         (i.e., new births) are being continuously added to the model.
...
UpdateAgentStates <- function(agent_states = NULL,
  ...
  if (init == TRUE) {
    ...
    clutch_data <- data.frame(nest_id=numeric(), parent_id=character(), date=character(),
                              x=numeric(), y=numeric(), offspring=numeric())
    sim$agents <- append(sim$agents, NamedList(all))
    sim$agents <- append(sim$agents, NamedList(clutch_data))
    return(sim)
  } else {
    if(add_agents == TRUE){
      new_input$id <- seq((max(sim$agents$input$id)+1),(max(sim$agents$input$id)+nrow(new_input)))
      ...
      if(ncol(sim$agents$input)<ncol(new_input)){
        new_cols <- colnames(new_input)[-(which(colnames(new_input) %in% colnames(sim$agents$input)))]
        new_data <- data.frame(matrix(nrow=nrow(sim$agents$input), ncol=length(new_cols)))
      ...
      if(nrow(current_step_data)==sim$pars$global$burnin){
        ...
        total_center <- colMeans(sim$agents$all[[current_agent]]$step_data[,c("x","y")],na.rm=T)
  ...
  if (init == TRUE) {
    ...
    for (i in 1:length(all)) {
      ...
      if(any(names(agent)=="step_data")){
        ...
        step_data <- data.frame(id=agent$states$id, class=agent$states$class,
                                datetime=start_datetime,
                                x=agent$states$start_x, y=agent$states$start_y)
        step_data_col <- ncol(step_data)
        if(!is.null(step_attributes)){
          step_data <- cbind(step_data,matrix(ncol=length(step_attributes),nrow=nrow(step_data),data=NA))
...
# CreateRedistributionKernel Function ----------------------------------------------

###  Combines multiple raster objects into a single, normalized redistribution kernel.
###  Usage: CreateRedistributionKernel(raster_stack, combine_method, adjust_distance,
###   current_location, base)
###  Arguments: raster_stack = a RasterStack object containing all raster objects to be combined
###             combine method = character. "product" takes the product of all rasters within
###               the RasterStack. "geometric" takes the geometric mean of all rasters within
###               the raster stack.
###             adjust_distance = logical. indicates whether or not the final redistribution
###               kernel should be adjusted based on expected cell counts.
###             current_location = numeric. the x and y coordinates of current location.
...
CreateRedistributionKernel <- function(raster_stack, combine_method, adjust_distance=TRUE,
                                       current_location, base){
  ...
  if(adjust_distance==TRUE){
    center <- cellFromXY(prob_raster,matrix(c(current_location[1],current_location[2]),nrow=1))
  ...
  adjusted_raster[cellFromXY(adjusted_raster,matrix(c(current_location[1],current_location[2]),nrow=1))]<-0
